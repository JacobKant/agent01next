...
### 5. Git Server (`git-server.ts`)

**Назначение**: Просмотр истории коммитов Git репозитория

**Инструменты**:
- `git_log` - Просмотр истории коммитов с фильтрацией

**Параметры**:
```typescript
{
  limit?: number      // Количество коммитов (по умолчанию 10, максимум 100)
  author?: string     // Фильтр по автору (имя или email)
  since?: string      // Коммиты после даты (формат: YYYY-MM-DD или '2 weeks ago')
  until?: string      // Коммиты до даты (формат: YYYY-MM-DD или '1 week ago')
  path?: string       // Путь к файлу/директории для фильтрации
  search?: string     // Поиск по сообщению коммита
}
```

**Пример использования**:
```
Пользователь: "Покажи последние 5 коммитов"
AI: [вызывает git_log с limit=5]
AI: [показывает историю коммитов]

Пользователь: "Найди коммиты автора 'John' за последние 2 недели"
AI: [вызывает git_log с author="John", since="2 weeks ago"]
AI: [показывает отфильтрованные коммиты]
```

**Возвращаемые данные**:
- Информация о коммитах (хеш, автор, дата, сообщение)
- Статистика изменений (количество измененных файлов)
- Детали изменений по каждому файлу

**Инструменты**:
- `git_branches` - Просмотр всех веток (локальных и удаленных)
- `git_current_branch` - Получение информации о текущей ветке

**git_branches параметры**:
```typescript
{
  includeRemote?: boolean  // Включить удаленные ветки (по умолчанию true)
}
```

**git_current_branch параметры**:
```typescript
{}  // Без параметров
```
...
```
Пользователь: "Покажи все ветки"
AI: [вызывает git_branches]
AI: [показывает список всех веток с указанием текущей]

Пользователь: "На какой ветке я сейчас?"
AI: [вызывает git_current_branch]
AI: [показывает текущую ветку и информацию о последнем коммите]
```

**Возвращаемые данные git_branches**:
- Текущая ветка
- Список локальных веток с последним коммитом
- Список удаленных веток (если включено)
- Указание текущей ветки

**Возвращаемые данные git_current_branch**:
- Название текущей ветки
- Информация о последнем коммите (хеш, сообщение, дата, автор)
- Статус несохраненных изменений
- Список измененных файлов (если есть)

**Инструменты**:
- `git_status` - Просмотр статуса рабочей директории

**git_status параметры**:
```typescript
{
  short?: boolean  // Короткий формат вывода (по умолчанию false)
}
```

**Пример использования git_status**:
```
Пользователь: "Какой статус репозитория?"
AI: [вызывает git_status]
AI: [показывает статус всех измененных файлов]

Пользователь: "Покажи краткий статус"
AI: [вызывает git_status с short=true]
AI: [показывает краткий список измененных файлов]
```

**Возвращаемые данные git_status**:
- Текущая ветка и отслеживаемая ветка (если есть)
- Количество коммитов впереди/позади удаленной ветки
- Сводка изменений (staged, unstaged, untracked, unmerged)
- Детальный список файлов с их статусами:
  - staged (добавлены в индекс)
  - unstaged (изменены, но не добавлены в индекс)
  - both (изменены и в staged, и в unstaged)
  - untracked (не отслеживаются)
  - unmerged (конфликты слияния)
- Для каждого файла: путь, статус, тип изменения (modified, added, deleted, renamed, copied, untracked, unmerged)
